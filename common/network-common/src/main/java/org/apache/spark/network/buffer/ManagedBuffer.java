...
public abstract class ManagedBuffer {
   ...
   */
  public abstract long size(); //返回数据的字节数
  ...
  public abstract ByteBuffer nioByteBuffer() throws IOException;  //将数据按照NIO的ByteBuffer类型返回。
   ...
   */
  public abstract InputStream createInputStream() throws IOException;  //将数据按照InputStream返回。

  /**
   * Increment the reference count by one if applicable.
   */
  public abstract ManagedBuffer retain();  //当有新的使用者使用些视图时，增加引用此视图的引用数。

  /**
   * If applicable, decrement the reference count by one and deallocates the buffer if the
   * reference count reaches zero.
   * 当有使用都不再使用此视图时，减少引用此视图的引用数。当引用数为0时释放缓冲区。
   */
   ...
   * 将缓冲区的数据转换为Netty的对象，用来将数据写到外部。此方法返回的数据类型要么是io.netty.buffer.ByteBuf，
   * 要么是io.netty.channel.FileRegion
   */
